defmodule Electric.Replication.PostgresConnectorSup do
  ...
  def init(conn_config) do
    ...
    inbound_mode = Connectors.inbound_mode(conn_config)

    migration_consumer_opts = [
      producer: postgres_producer,
      refresh_subscription: inbound_mode == :logical_replication
    ]
    ...
    children = [
      ...
      {SatelliteCollectorProducer,
       name: SatelliteCollectorProducer.name(), inbound_mode: inbound_mode},
      %{
      ...
      if inbound_mode == :logical_replication do
